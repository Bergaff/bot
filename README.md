...
`npm test` — 409 тестов, `npm run typecheck` — `tsc --noEmit`.
...
Клиент (этап 3) — **только чтение** открытой вкладки: ни кликов, ни отправки, ни реакций. Чат не из белого списка не читается вовсе (запись `чат :: рум` ограничивает один топик форум-чата); пассажирские попутки и болтовня отсеиваются ещё в браузере (бандл `src/parser.ts` → `extension/vendor/parser.js`); по умолчанию стоит режим подтверждения; дубли режет локальный лог `chatId:messageId`, а на сервере — `tg_seen`. Установка, настройки, коды ошибок и починка селекторов — в [`extension/README.md`](extension/README.md).
...
| `tests/` | 409 тестов + фикстуры разметки, мини-DOM и мини-браузер | 1–7 |
...
| `tests/sourcelink.test.ts` | `listingSourceLink()`: `web:durov`+528 → `t.me/durov/528`, `ext:-100123`+9001 → служебная `t.me/c/123/9001`, обычная группа и личный чат из расширения → `null`, приоритет `chat_links`, карточка `formatListing` |
