...
| --- | --- | --- |
| `GET /api/extension/config` | `INGEST_TOKEN` | расширение забирает белый список чатов и румов, интервал, паузу и темп автообхода |
| `POST /api/extension/heartbeat` | `INGEST_TOKEN` | отметка «аккаунт подключён»: чат, рум, счётчики, состояние, ошибки, ход автообхода |
| `GET /api/admin/extension` | `ADMIN_API_TOKEN` | панель: кто подключён + текущие настройки |
| `PUT /api/admin/extension/config` | `ADMIN_API_TOKEN` | панель: задать белый список/интервал/паузу/автообход |
...
поля автообхода живут в том же JSON `ext:config`/`ext:clients`, старых значений не ломают
(отсутствие поля = `null` = «клиент возьмёт своё значение по умолчанию»).

#### Автообход чатов в настройках (`ExtensionConfig`)

Расширение умеет само открывать чаты и румы из белого списка по кругу (детали и предупреждение
о риске — `extension/README.md`, раздел «Автообход чатов и румов»). Панель задаёт темп, клиент
его забирает тем же `GET /api/extension/config` / ответом на отметку:

| Поле | Границы в `normalizeExtensionConfig()` | Смысл |
| --- | --- | --- |
| `autoWalk` | `boolean` или `null` | включить обход; **по умолчанию `null`/`false`** — клиент сам его не включает |
| `walkReadsPerChat` | 1…20 | проходов чтения на чат, прежде чем перейти дальше |
| `walkMinSec` | 10…3600 | нижняя граница случайной паузы между переходами, с |
| `walkMaxSec` | 10…7200, и не меньше `walkMinSec` | верхняя граница паузы |
| `walkMaxPerHour` | 1…600 | предел переходов в час |
| `walkIdleGuardSec` | 0…1800 | пока пользователь активен во вкладке — чат не переключаем |

Границы те же, что в клиенте (`extension/core.js → withDefaults`): панель не может задать темп,
который клиент сочтёт безумным. В отметке клиент присылает объект `walk`
(`ExtensionWalkRef`: `on`, `plan`, `current`, `next`, `nextInSec`, `switchesHour`,
`switchesLimit`, `note`, `log[]`) — `recordExtensionClient()` кладёт его в `ext:clients`,
`GET /api/admin/extension` отдаёт как есть, а панель рисует в карточке аккаунта
(`parcel/app-auto-collect.js → extWalkBlock()`).
...
- автообход: галочка «автообход чатов» и темп (проходов на чат, пауза от/до, переходов в час)
  уходят тем же `PUT`; в карточке аккаунта видно, где обход сейчас, куда пойдёт дальше, через
  сколько секунд переход, сколько переходов за час и чем кончились последние из них
  (`extWalkBlock()`);
