 ...
 */
import type { Env, ListingType } from './types.ts';
import { normalizeCity, } from './parser.ts';
import { dedupeDescription, isRussianCity, normalizeContacts } from './util.ts';

/** Дневной лимит ИИ-вызовов бота — страховка от неожиданного счёта. */
const DAILY_AI_LIMIT = 300;

/** Дневной лимит ИИ-вызовов авто-сбора (вариант A) — переменная COLLECT_AI_DAILY_LIMIT. */
const COLLECT_AI_DAILY_LIMIT_DEFAULT = 100;

/**
 * Чей бюджет ИИ тратим. У каждого канала свой счётчик в KV, поэтому
 * авто-сбор (cron по публичным чатам) и приём из браузерного расширения
 * не съедают дневной лимит бота — и наоборот.
 *
 *   bot       → ai:day:<дата>          (как было, 300/день)
 *   collect   → ai:collect:day:<дата>  (вариант A, COLLECT_AI_DAILY_LIMIT)
 *   extension → ai:ingest:day:<дата>   (вариант B, тот же лимит)
 */
...
/** Сегодня по Москве/Минску (UTC+3) — сутки счётчика совпадают с сутками бота. */
function aiDay(): string {
  return new Date(Date.now() + 3 * 3600 * 1000).toISOString().slice(0, 10);
}

function aiQuotaKey(scope: AiQuotaScope): string {
  const day = aiDay();
  return scope === 'bot' ? `ai:day:${day}` : scope === 'collect' ? `ai:collect:day:${day}` : `ai:ingest:day:${day}`;
}

function quotaLimitOf(env: Env, scope: AiQuotaScope): number {
  if (scope === 'bot') return DAILY_AI_LIMIT;
  const raw = parseInt(env.COLLECT_AI_DAILY_LIMIT ?? '', 10);
  return Number.isFinite(raw) && raw > 0 ? raw : COLLECT_AI_DAILY_LIMIT_DEFAULT;
}

/** Сколько ИИ-вызовов канал уже потратил сегодня (ничего не инкрементирует). */
export async function aiQuotaUsed(env: Env, scope: AiQuotaScope = 'bot'): Promise<number> {
  const raw = await env.KV.get(aiQuotaKey(scope));
  const count = raw ? parseInt(String(raw), 10) : 0;
  return Number.isFinite(count) && count > 0 ? count : 0;
}

/** Хватит ли квоты на один вызов (счётчик не трогает) — для dryRun и отчётов. */
export async function aiQuotaAvailable(env: Env, scope: AiQuotaScope = 'bot'): Promise<boolean> {
  return (await aiQuotaUsed(env, scope)) < quotaLimitOf(env, scope);
}

/** Дневная квота ИИ-вызовов (KV-счётчик) — чтобы счёт не удивил.
 *  consume=false — только проверить (dryRun: квоту не тратим). */
async function aiQuotaOk(env: Env, scope: AiQuotaScope = 'bot', consume = true): Promise<boolean> {
  const key = aiQuotaKey(scope);
  const raw = await env.KV.get(key);
  const count = raw ? parseInt(String(raw), 10) : 0;
  if (count >= quotaLimitOf(env, scope)) return false;
  if (consume) await env.KV.put(key, String(count + 1), { expirationTtl: 2 * 86400 });
  return true;
}

/** Остаток дневной квоты канала — сборщик планирует им глубину обхода. */
export async function aiQuotaLeft(env: Env, scope: AiQuotaScope = 'bot'): Promise<number> {
  return Math.max(0, quotaLimitOf(env, scope) - (await aiQuotaUsed(env, scope)));
}
...
export async function aiExtractListing(
  env: Env,
  text: string,
  opts: { scope?: AiQuotaScope; consumeQuota?: boolean } = {}
): Promise<AiFields[]> {
  if (!env.AI_API_KEY) return [];
  const scope = opts.scope ?? 'bot';
  if (!(await aiQuotaOk(env, scope, opts.consumeQuota !== false))) return [];
