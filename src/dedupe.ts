/**
 * КОПИЯ src/dedupe.ts из Bergaff/parcel (ветка arena/01a0a5c0-parcel, коммит e037b3f) —
 * отличается только `.ts` в импортах. После переноса скраппера в parcel этот файл
 * удаляется: там он уже есть, а ingest.ts/collect.ts просто вызывают createListingSafe().
 *
 * Зачем он здесь: tg_seen защищает от повторов ОДНОГО сообщения, а водитель пишет
 * «20 сентября Варшава — Минск» каждый день новыми сообщениями. Без сравнения по смыслу
 * авто-сбор наплодил бы пять одинаковых заявок — ровно та боль, которую parcel закрыл.
 *
...
import { uniqueContacts } from './util.ts';
